...
class Sales extends Secure_Controller
{
	...
	public function set_sales_print_as_pdf()
	{
		$this->sale_lib->set_sales_print_as_pdf($this->input->post('print_as_pdf'));
	}
	...
	public function set_cart_tab()
	{
		$this->sale_lib->set_cart_tab($this->input->post('card_tab'));
	}
	...
	{
		$data = array();
		$this->load->model('Addsp_sale');
		$model = $this->Addsp_sale;
		$data['addsp_sales'] = $this->Addsp_sale->get_all()->result_array();
		$this->load->view('sales/addsp', $data);
	}
	...
	public function complete()
	{
		...
		if($this->sale_lib->get_cart_as_edit() == 'edit')
		{
			$sale_ids = $sale_id == -1 ? $this->input->post('ids') : array($sale_id);
			if($this->Sale->delete_list($sale_ids, $employee_id, TRUE))
			{		
			    $this->Sale->restore_list($sale_ids, $employee_id, TRUE);
		    }
		}
		...
		$data['print_as_pdf'] = $this->sale_lib->is_sales_print_as_pdf();
		...
		if($this->sale_lib->is_invoice_mode() || $pos_invoice)
		{
			...
			else
			{
				...
				$data['sale_id_num'] = $this->Sale->save($sale_id, $data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'], $invoice_number, $work_order_number, $quote_number, $sale_type, $data['payments'], $data['dinner_table'], $data['taxes'], $stock_location);
		...
		elseif($this->sale_lib->is_quote_mode())
		{
			...
			else
			{
				...
				$this->load->view('invoice/quote', $data);
				$this->sale_lib->clear_mode();
				$this->sale_lib->clear_all();
			}
		}
	...
	private function _reload($data = array())
	{
		...
		$data['cart'] = $this->sale_lib->get_cart();	
		$data['cart_tab'] = $this->sale_lib->get_cart_tab();
		...
		if (!$this->config->item('sales_focus_on_item'))
		{
			$data['tab_location'] = $this->tab_lib->get_tab_location();
		}
		else
		{
			$data['tab_location'] = '#item';
		}
		...
		if($customer_info)
		{
			...
			{
			    $data['payment_options'] = $this->Sale->get_payment_options(TRUE, TRUE, TRUE);
			}
			else
			{
				$data['payment_options'] = $this->Sale->get_payment_options(TRUE, FALSE, TRUE);
			}
		}
		...
		$data['sales_price_allowed'] = $this->Employee->has_grant('sales_rate', $this->Employee->get_logged_in_employee_info()->person_id);
		...
		$data['pos_mode'] = $data['mode'] == 'sale_invoice' || $data['mode'] == 'return';
		...
		$data = $this->xss_clean($data);
		$this->load->view("sales/register_pos", $data);
	}
	
	public function customer_display()
	{
		$data['shows'] = $this->_slide_shows();
		$this->load->view("sales/register_pos_dispaly", $data);
	}	
	
	public function customer_display1()
	{
		$sale_id = $this->session->userdata('sale_id');
		if($sale_id == '')
		{
		...
		$this->load->view("pos/display", $data);
	}	
	...
	public function receiptbt($sale_id)
	{
        ...
        foreach($data['cart'] as $line=>$item)
		{
			...
			$apps["item"] = $item['name'];
			$apps["quantity"] = to_quantity_decimals($item['quantity']);
	...
	public function generate_arabic_bill($sale_id)
	{
		$data = $this->_load_sale_data($sale_id);
		$this->load->view('sales/receipt_arabic', $data);
		$this->sale_lib->clear_all();
	}
	...
	public function edit_sale($update_inventory = TRUE)
	{
		...
		else
		{
			...
			$this->sale_lib->set_cart_as_edit('edit');
	...
	public function suspend()
	{
		...
		$data = array();
		$sales_taxes = array();
		if($this->Sale->save($sale_id, $sale_status, $cart, $customer_id, $employee_id, $comment, $invoice_number, $work_order_number, $quote_number, $sale_type, $payments, $dinner_table, $sales_taxes, $stock_location) == '-1')
		{
	...
	{
		$data = array();
		$data['reprint_sales'] = $this->Sale->get_last_sale_ids()->result_array();
		$this->load->view('sales/reprint', $data);
	}
	
	function reprint_recipt()
	{
		$sale_id = $this->input->post('sale_id');
        ...
        <script language="javascript">
	        window.location = '<?php echo base_url();?>index.php/sales/receipt/<?php echo $sale_id;?>'
	    </script>
		<?php
		}
    }	
	public function get_sale_item_info($item_id, $customer_id = -1)
	{
		$data['costs'] = $this->Receiving->get_cost_item_info($item_id)->result_array();;
        $data['cost'] = $this->Item->get_info($item_id)->cost_price;
		$data['minimum_price'] = $this->Item->get_info($item_id)->minimum_price;
		$data['sales'] = $this->Sale->get_sale_item_info($item_id, $customer_id)->result_array();;
		$data['sales_all'] = $this->Sale->get_sale_all_item_info($item_id)->result_array();;
		$this->load->view('sales/cost', $data);
	}
	...
	public function get_item_by_cat()
    {
        ...
        $suggestions = array();
          ...
          foreach($this->Item->get_all_category_item($category)->result() as $row){
                   $suggestions[] = array('value' => $row->item_id, 'label' => $this->Item->get_search_suggestion_label($row), 'icon'=> $row->pic_filename);
          }
	...
	public function get_cat()
    {
        ...
        $items = $this->Item->get_all_category();
        
		$suggestions = array();
        
        foreach ( $items->result() as $result) {
            $suggestions[] = array('category' => $result->category);
        }
	...
	public function play_slideshow()
	{
		$data['shows'] = $this->_slide_shows();
		$this->load->view('pos/show', $data);
	}
		...
		public function pos_items()
	{
        ...
        echo ' <td>'. to_currency($item['price']).'</td>';
	    echo ' <td>'. to_quantity_decimals($item['quantity']).'</td>';
		echo ' <td>'. to_currency($item['discount']).'</td>';
		echo ' <td>'. to_currency($item['discounted_total']).'</td>';
        echo '<tr>';
		}
	}

	
	public function pos_totals()
	{
		$totals = $this->sale_lib->get_totals();
		...
		{
            $total_tax += $sales_tax['sale_tax_amount'];
		}
				
		if($data['cash_rounding'])
		{
			$total = $totals['cash_total'];
			$amount_due = $totals['cash_amount_due'];
		}
		else
		{
			$total = $totals['total'];
        ...
        echo ' <table style="width:100%; float:right; padding:5px; color:#000; background: #FFF;" id="totalTable"> ';
        echo ' <tbody> ';
        echo ' <tr> ';
        echo '  <td style="border-left: 1px solid #ddd; padding-left:10px; text-align:left; font-weight:normal;"> ';
		echo $this->lang->line('sales_quantity_of_items',$item_count) .'</td>';
        echo ' <td style="text-align:right; padding-right:10px; font-weight:bold;"><span id="count">'. $total_units .'</span></td>';
        echo ' </tr> ';
		
		echo ' <tr> ';
		echo ' <td style="border-left: 1px solid #ddd; padding-left:10px; text-align:left; ">'. $this->lang->line('sales_sub_total') .'</td>';
		echo ' <td style="text-align:right; padding-right:10px; font-weight:bold;">';
		echo ' <span id="ds_con">'. to_currency($subtotal) .'</span></td>';
		echo ' <td style="padding-left:10px; text-align:left; font-weight:normal;">Tax</td>';
		echo ' <td style="border-right: 1px solid #ddd; text-align:right; padding-right:10px; font-weight:bold;"><span id="ts_con">'. to_currency($total_tax) .'</span></td>';
        echo ' </tr> ';
		
		echo ' <tr> ';
		echo ' <td colspan="2" style="padding: 5px 0px 5px 10px; text-align:left; font-size: 1.4em; border: 1px solid #333; font-weight:bold; background:#333; color:#FFF;">';
		echo $this->lang->line("sales_total") .' </td>';
		echo ' <td colspan="2" style="text-align:right; padding:5px 10px 5px 0px; font-size: 1.4em; border: 1px solid #333; font-weight:bold; background:#333; color:#FFF;">';
		echo ' <span id="total-payable">'. to_currency($total).'</span></td>';
		
        echo ' </tr> ';
        echo ' </table> ';
	}
	
		public function pos_items_tmp()
	{
		$cart = $this->sale_lib->get_cart();
		...
		echo ' <table class="table table-striped table-dark"> ';
  		echo ' <thead>';
    	echo ' <tr> ';
      	echo ' <th scope="col">'. $this->lang->line('sales_item_name').'</th>';
		echo ' <th scope="col">'. $this->lang->line('sales_price').'</th>';
		echo ' <th scope="col">'. $this->lang->line('sales_quantity').'</th>';
		echo ' <th scope="col">'. $this->lang->line('sales_discount').'</th>';
		echo ' <th scope="col">'. $this->lang->line('sales_total').'</th>';
    	echo ' </tr>';
  		echo ' </thead> ';
  		echo ' <tbody id="cart_contents">';

		if(count($cart) == 0)
		{
			echo ' <tr> ';
				echo ' <td colspan="8">';
					echo " <div class='alert alert-dismissible alert-info'>". $this->lang->line('sales_no_items_in_cart') ."</div>";
				echo ' </td>';
			echo ' </tr>';
		}
	...
	public function create_temp(array $inputs)
	{
	...
	public function work_orders()
	{
		...
		else
		{
			...
			$this->load->view('sales/manage_work', $data);
		}
	}
	
	public function work_orders_search()
	{
		$search = $this->input->get('search');
		$limit = $this->input->get('limit');
		$offset = $this->input->get('offset');
