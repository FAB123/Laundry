...
class Expenses extends Secure_Controller
{
	public function __construct()
	{
		...
		$this->load->library('expenses_lib');
	}
	...
	public function index()
	{
		...
		$data['hdate'] = $hdate;
		$data['stock_locations'] = $this->Stock_location->get_allowed_locations('expense');
		$data['stock_location'] = $this->xss_clean($this->expenses_lib->get_expenses_location());
	...
	public function search()
	{
		...
		$this->expenses_lib->set_expenses_location($this->input->get('stock_location'));

		// check if any filter is set in the multiselect dropdown
		$filledup = array_fill_keys($this->input->get('filters'), TRUE);
		$filters = array_merge($filters, $filledup);
		//$location_id = $this->input->get('stock_location');
		$location_id = $this->expenses_lib->get_expenses_location();
		$expenses = $this->Expense->search($search, $filters, $location_id, $limit, $offset, $sort, $order);
		$total_rows = $this->Expense->get_found_rows($search, $filters, $location_id);
		$payments = $this->Expense->get_payments_summary($search, $filters, $location_id);
	...
	public function save($expense_id = -1)
	{
		...
		$amounts = parse_decimals($this->input->post('amount'));

		$expense_data1 = array(
			    'thedate'=> $date_formatter->format('Y-m-d H:i:s'),
				'description'=>'EXP '.$expense_id,
		        'type'=>'Expense',
				'employee_id' =>  $this->input->post('employee_id'),
				'location_id' => $this->input->post('stock_location'),
		...
		if($this->Expense->save($expense_data, $expense_data1, $expense_id))
		{
			$expense_data = $this->xss_clean($expense_data);
			$expense_data1 = $this->xss_clean($expense_data1);
