...
class Config extends Secure_Controller
{
	...
	*/
	private function _themes()
	{
		...
		$dir = new DirectoryIterator('dist/themes');
	...
	private function _spinners()
	{
		$themes = array();

		// read all themes in the dist folder
		$dir = new DirectoryIterator('images/loader');
		...
		asort($spinners);
        //$spinners = scandir('images/loader');
		return $spinners;
	}
	...
	public function index()
	{
		...
		$data['barcode_methods'] = $this->barcode_lib->get_list_barcode_method();
		...
		$trial_time = $this->reg_lib->check_version();
		$data['trial_time'] = (15 - $trial_time);
		$data['activated'] = $this->Appconfig->get('version_info');
	...
	public function save_info()
	{
		...
		$batch_save_data = array(
			...
			'company_ar' => $this->input->post('company_ar'),
			...
			'return_policy_ar' => $this->input->post('return_policy_ar')
		);
	...
	public function save_general()
	{
		$batch_save_data = array(
			...
			'sales_focus_on_item' => $this->input->post('sales_focus_on_item') != NULL,
			'disable_add_duplicate' => $this->input->post('disable_add_duplicate') != NULL,
	...
	*/
	...
	public function save_barcode()
	{
		$batch_save_data = array(
			...
			'barcode_fourth_row' => $this->input->post('barcode_fourth_row'),
	...
	public function save_receipt()
	{
		$batch_save_data = array (
			...
			'arabic_support' => $this->input->post('arabic_support') != NULL,
			...
			'receipt_show_watermark' => $this->input->post('receipt_show_watermark') != NULL,
			'receipt_show_amount_change' => $this->input->post('receipt_show_amount_change') != NULL,
			'receipt_show_payments' => $this->input->post('receipt_show_payments') != NULL,
			...
			'core_api_key' => $this->input->post('core_api_key')
		);
	...
	public function save_invoice()
	{
		$batch_save_data = array (
			...
			'quote_default_comments_ar' => $this->input->post('quote_default_comments_ar'),
	...
	public function save_activation()
	{
		...
		$address = $this->reg_lib->reg_key();
		$address = $this->reg_lib->crypter($address);
		
		if($address == $activation_key)
		{
            $batch_save_data = array(
	...
	public function backup_db()
	{
		...
		if($this->Employee->has_module_grant('config', $employee_id))
		{
			...
			$prefs = array(
				'format' => 'zip',
				'filename' => 'SalesTime.sql'
			);

			$backup = $this->dbutil->backup($prefs);

			$file_name = 'SalesTime-' . date("Y-m-d-H-i-s") .'.zip';
