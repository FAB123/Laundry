...
class Cashups extends Secure_Controller
{
	...
	public function view($cashup_id = -1)
	{
			...
			floatval($cash_ups_info->closed_amount_check) == 0)
		{
			...
			foreach($reports_data as $row)
			{
				...
				if($row['payment_type'] == $this->lang->line('sales_as_credit'))
				{
					$cash_ups_info->closed_amount_credit += $this->xss_clean($row['amount']);
				}
				elseif($row['payment_type'] == $this->lang->line('sales_due'))
				{
					$cash_ups_info->closed_amount_due += $this->xss_clean($row['amount']);
				}
				elseif($row['payment_type'] == $this->lang->line('sales_debit') || 
						$row['payment_type'] == $this->lang->line('sales_credit'))
				{
					$cash_ups_info->closed_amount_card += $this->xss_clean($row['amount']);
				}
				elseif($row['payment_type'] == $this->lang->line('sales_check'))
				{
					$cash_ups_info->closed_amount_check += $this->xss_clean($row['amount']);
				}
			}
			
			$this->load->model('reports/Summary_receiving_payments');
			$receiving_reports_data = $this->Summary_receiving_payments->getData($inputs);
			
			foreach($receiving_reports_data as $row)
			{
				if($row['payment_type'] == $this->lang->line('sales_cash'))
				{
					$cash_ups_info->total_receiving_cash += $this->xss_clean($row['amount']);
				}
			}
	...
	public function save($cashup_id = -1)
	{
		...
		$cash_up_data = array(
			...
			'closed_amount_credit' => parse_decimals($this->input->post('closed_amount_credit')),
			'closed_amount_total' => parse_decimals($this->input->post('closed_amount_total')),
			'total_expanse_cash' => parse_decimals($this->input->post('total_expanse_cash')),
			'total_receiving_cash' => parse_decimals($this->input->post('total_receiving_cash')),
	...
	*/
	public function ajax_cashup_total()
	{
		...
		$total = $this->_calculate_total($open_amount_cash, $transfer_amount_cash, $closed_amount_due, $closed_amount_cash, $closed_amount_card, $closed_amount_check, $closed_amount_credit);
	...
	Calculate total
	*/
	private function _calculate_total($open_amount_cash, $transfer_amount_cash, $closed_amount_due, $closed_amount_cash, $closed_amount_card, $closed_amount_check, $closed_amount_credit, $total_expanse_cash, $total_receiving_cash)
	{
		return ($closed_amount_cash - $open_amount_cash - $transfer_amount_cash + $closed_amount_due + $closed_amount_card + $closed_amount_check + $closed_amount_credit + $total_expanse_cash + $total_receiving_cash);
	}
}
?>
