...
class Customers extends Persons
{
	...
	public function index()
	{
		...
		$data['stock_locations'][All] = $this->lang->line('location_all');
		$data['stock_location'] = $this->xss_clean($this->customer_lib->get_customer_location());
	...
	*/
	public function search()
	{
		...
		$this->customer_lib->set_customer_location($this->input->get('stock_location'));
		
		$stock_location  = $this->customer_lib->get_customer_location();

		$customers = $this->Customer->search($search, $limit, $offset, $sort, $order, $stock_location);
	...
	*/
	...
	{
		$suggestions = $this->xss_clean($this->Customer->get_search_suggestions($this->input->get('term'), FALSE, TRUE));

		echo json_encode($suggestions);
	}

	public function customer_suggest()
	{
		$stock_locations = $this->Stock_location->get_allowed_locations('customers');
		$suggestions = array();
		foreach($stock_locations as $stock_location => $value)
		{
			$suggestions = array_merge($suggestions, $this->xss_clean($this->Customer->get_search_suggestions($this->input->get('term'), $stock_location, TRUE)));
		}
	...
	*/
	public function save($customer_id = -1)
	{
		...
		$customer_data = array(
			...
			'Vat_no' => $this->input->post('Vat_no') == '' ? NULL : $this->input->post('Vat_no')
		);
