...
class Receiving_lib
{
	...
	public function add_item($item_id, $quantity = 1, $item_location = NULL, $discount_type = 0, $discount = 0, $price = NULL, $description = NULL, $serialnumber = NULL, $receiving_quantity = NULL, $receiving_id = NULL, $include_deleted = FALSE)
	{
		...
		{
			if($this->CI->config->item('disable_add_duplicate'))
		    {
			   return FALSE;
		    }
			else
	...
	public function get_taxes()
	{
		...
		$sales_taxes = array();
		//Do not charge sales tax if we have a customer that is not taxable
		if($supplier->taxable or $supplier_id == -1)
		{
			foreach($this->get_cart() as $line => $item)
			{
				...
				foreach($tax_info as $tax)
				{
					...
					$tax_basis = $this->get_item_total($item['quantity'], $item['price'], $item['discount'], $item['discount_type'], $item['receiving_quantity']);
					$tax_amount = 0;

					//if($this->CI->config->item('customer_sales_tax_support') == '0')
					{
