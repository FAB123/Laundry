...
class Sale_lib
{
	...
	public function get_cart_as_edit()
	{
		if(!$this->CI->session->userdata('sales_cart_as_edit'))
		{
			$this->set_cart_as_edit(array());
		}

		return $this->CI->session->userdata('sales_cart_as_edit');
	}
	
	public function set_cart_as_edit($cart_data)
	{
		$this->CI->session->set_userdata('sales_cart_as_edit', $cart_data);
	}

	public function empty_cart_as_edit()
	{
		$this->CI->session->unset_userdata('sales_cart_as_edit');
	}
	...
	public function set_sales_print_as_pdf($print_as_pdf)
	{
		echo $this->CI->session->set_userdata('print_receipt_as_pdf', $print_as_pdf);
		return $this->CI->session->set_userdata('print_receipt_as_pdf', $print_as_pdf);
	}
	...
	public function is_sales_print_as_pdf()
	{
		if($this->CI->config->item('print_receipt_as_pdf') == 'always')
		{
			return TRUE;
		}
		elseif($this->CI->config->item('print_receipt_as_pdf') == 'never')
		{
			return FALSE;
		}
		else // remember last setting, session based though
		{
			return ($this->CI->session->userdata('print_receipt_as_pdf') == 'true' ||
					$this->CI->session->userdata('print_receipt_as_pdf') == '1');
		}
	}
	...
	public function set_cart_tab($value)
	{
		$this->CI->session->set_userdata('cart_tab', $value);
	}

	public function get_cart_tab()
	{
		if(!$this->CI->session->userdata('cart_tab'))
		{
			$this->set_cart_tab('sale_tab');
		}
		return $this->CI->session->userdata('cart_tab');
	}

	public function clear_cart_tab()
	{
		$this->CI->session->unset_userdata('cart_tab');
	}
	...
	public function add_item(&$item_id, $quantity = 1, $item_location, $discount = 0, $discount_type = 0, $price_mode = PRICE_MODE_STANDARD, $kit_price_option = NULL, $kit_print_option = NULL, $price_override = NULL, $description = NULL, $serialnumber = NULL, $sale_id = NULL, $include_deleted = FALSE, $print_option = NULL )
	{
		...
		{
			if($item_info->low_sell_item_id == $item_info->item_id)
			{
				return FALSE;
			}
		}
		...
		if($price_mode == PRICE_MODE_STANDARD)
		{
			...
			if($this->get_customer()>0)
		    {
                $cust_type = $this->CI->Customer->get_info($this->get_customer())->customer_type;
            }
            $price = ($this->get_customer()>0)?$this->CI->Item->get_info($item_id)->$cust_type:$this->CI->Item->get_info($item_id)->unit_price;
		...
		foreach($tax_info as $tax)
		{
			...
			$item_tax = $this->item_taxs($tax_percent, $price);
			$item_taxs += $item_tax;
		}
		...
		if($this->CI->Employee->has_grant('sales_disablenegative', $this->CI->Employee->get_logged_in_employee_info()->person_id))
		{
			if($this->CI->Item_quantity->get_item_quantity($item_id, $item_location)->quantity < 1)
		    {
		    	return FALSE;
		    }
		}
	...
	//Item already exists and is not serialized, add to quantity
		...
		elseif($this->CI->config->item('disable_add_duplicate'))
		{
			return FALSE;
		}
		else
	...
	{
		$items = $this->get_cart();
		$id = $this->get_item_id($line);
		$minimum_price = $this->CI->Item->get_info($id)->minimum_price;
		if(isset($items[$line]))
		{
			$line = &$items[$line];
			...
			$the_total = $this->get_item_total($quantity, $price, $discount, $line['discount_type'], TRUE);
			$line['discounted_total'] = $the_total;
			$mode = $this->get_mode();
		
			$min_price = bcmul($minimum_price, abs($quantity));
			if ($min_price > abs($the_total))
			{
				return false;
			}
			else 
	...
	public function item_taxs($tax_percentage, $price)
	{
		if($this->CI->config->item('tax_included'))
		{
			$tax_fraction = bcadd(100, $tax_percentage);
