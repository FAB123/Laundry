...
function addaddress_ar( $company, $address, $tel, $vat )
{
	...
	//Positionnement en bas
	$this->SetXY( $x1, $y1 );
	$this->SetFont('aefurat','B',20);
	$length = $this->GetStringWidth( $company );
	//$this->Cell( $length, 2, $company);
	$this->Cell($length,2, "$company " , 0, 0, "C");
	...
	$lignes = $this->sizeOfText( $address, $length) ;
	//$this->MultiCell($length, 4, $address);
	$this->MultiCell(82, 5, $address, 0, "C");
	...
	$this->Cell(82,2, "تليفون : ". $tel , 0, 0, "C");
	$this->SetXY( $x1, $y1 + 27 );
	$this->SetFont('aefurat','',12);
	$length = $this->GetStringWidth( $vat );
	//$this->Cell( $length, 2, "رقم ضريبة : ". $vat);
	$this->Cell(82,2, "رقم ضريبة : ". $vat , 0, 0, "C");
	$this->setRTL(false);
}
...
function addtotalcash($sub, $total)
{
	$r1  = $this->w - 75;
	$r2  = $r1 + 65;
	$y1  = $this->h - 38;
...
function total_qty($qty)
{
	$r1  = $this->w - 50;
	$r2  = $r1 + 40;
	$y1  = $this->h - 58;
	$y2  = $y1+20;
	$this->RoundedRect($r1, $y1, ($r2 - $r1), 35, 2.5, 'D');
    ...
    $this->Text($r1+12, $y1+5, 'Total Qty');
    $this->Text($r1+15, $y1+18, $qty.' Pcs');
}

function shoper_sign()
{
	$this->SetFont( "dejavusans", "B", 8);
	$r1  = 10;
	$r2  = $r1 + 70;
	...
	$y2  = $y1+35;
	$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2-$y1), 2.5, 'D');
	$this->Text($r1+15, $y1+3, 'Storekeeper Signature');
}

function salesman_sign()
{
	$this->SetFont( "dejavusans", "B", 8);
	$r1  = 85;
	$r2  = $r1 + 70;
	...
	$y2  = $y1+35;
	$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2-$y1), 2.5, 'D');
	$this->Text($r1+15, $y1+3, 'Receiver Signature');
}
