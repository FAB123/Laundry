...
class Expense extends CI_Model
{
	...
	*/
	...
	Gets rows
	*/
	public function get_found_rows($search, $filters, $location_id)
	{
		return $this->search($search, $filters, $location_id, 0, 0, 'expense_id', 'asc', TRUE);
	}

	/*
	Searches expenses
	*/
	public function search($search, $filters, $location_id, $rows = 0, $limit_from = 0, $sort = 'expense_id', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
	...
	*/
	public function save(&$expense_data, $expense_data1, $expense_id = FALSE)
	{
		if(!$expense_id || !$this->exists($expense_id))
		{
			if($this->db->insert('expenses', $expense_data))
			{
				$idfs = $this->db->insert_id();
				$expense_data['expense_id'] = $idfs;
				$expense_data1['description'] = 'EXP '.$idfs;
				$this->db->insert('accounts', $expense_data1);
		...
		$this->db->where('description', 'EXP '.$expense_id);
		$this->db->update('accounts', $expense_data1);
	...
	*/
	public function delete_list($expense_ids)
	{
		...
		{
			$this->db->where_in('description', 'EXP '.$expense_id);
			$success = $this->db->update('accounts', array('deleted'=>1));
		}
	...
	*/
	public function get_payments_summary($search, $filters, $location_id)
	{
		// get payment summary
