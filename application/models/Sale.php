...
class Sale extends CI_Model
{
	...
	{
		// get payment summary
		$this->db->select('sales_payments.payment_type, COUNT(payment_amount) AS count, SUM(payment_amount) AS payment_amount');
		...
		if($filters['only_cash'] != FALSE)
		{
			$this->db->like('sales_payments.payment_type', $this->lang->line('sales_cash'));
		}

		if($filters['only_due'] != FALSE)
		{
			$this->db->like('sales_payments.payment_type', $this->lang->line('sales_due'));
		}

		if($filters['only_check'] != FALSE)
		{
			$this->db->like('sales_payments.payment_type', $this->lang->line('sales_check'));
		}
	 ...
	 */
	public function save($sale_id, &$sale_status, &$items, $customer_id, $employee_id, $comment, $invoice_number, $work_order_number, $quote_number, $sale_type, $payments, $dinner_table, $sales_taxes, &$stock_location)
	{
		if($sale_id != -1)
		{
		...
		foreach($items as $line=>$item)
		{
			...
			if($cur_item_info->stock_type == HAS_STOCK && $sale_status == COMPLETED)
			{
				...
				if($cur_item_info->item_type == ITEM_PACK)
				{
					$trans_pack_quantity = $cur_item_info->qty_per_pack;
					$trans_item_id = $cur_item_info->low_sell_item_id;
					$trans_item_quantitys = $this->Item_quantity->get_item_quantity($trans_item_id, $item['item_location']);;
					$trans_item_quantity = $trans_pack_quantity * $item['quantity'];
					
					$this->Item_quantity->save(array('quantity'	=> $trans_item_quantitys->quantity - $trans_item_quantity,
					'item_id'		=> $trans_item_id,
					'location_id'	=> $item['item_location']), $trans_item_id, $item['item_location']);
					
					$sale_remarks = '[PACK] POS '.$sale_id;
     				$inv_data = array(
	    				'trans_date'		=> date('Y-m-d H:i:s'),
	    				'trans_items'		=> $trans_item_id,
	    				'trans_user'		=> $employee_id,
	    				'trans_location'	=> $item['item_location'],
	    				'trans_comment'		=> $sale_remarks,
	    				'trans_inventory'	=> -$trans_item_quantity
		    		);
	    ...
	    foreach($payments as $payment_id=>$payment)
		{
			if ( substr( $payment['payment_type'], 0, strlen( $this->lang->line('sales_as_credit') ) ) == $this->lang->line('sales_as_credit') )
			{
				$credit = $payment['payment_amount'];
    		}
		}
		//$amount = $this->sale_lib->get_total();
		
			if($this->config->item('tax_included'))
			{
				//$subtotal=$this->sale_lib->calculate_subtotal(false, true);
			}
			else
			{
				//$subtotal=$this->sale_lib->calculate_subtotal();
			}
		...
		{
			$get_total_tax = $this->sale_lib->get_taxes();
			foreach($get_total_tax as $tax_group_index=>$sales_tax)
			{
				$tax_total += $sales_tax['sale_tax_amount'];
			}

			//$total_tax = bcsub($amount, $subtotal);
			$saled_amount = $this->sale_lib->get_total();
			$payed = bcsub($amount, $credit);
			$payed = round($payed);
			if(empty($tax_total))
			{
				$tax_total = '0';
			...
			$sales_cash_data = array(
				'thedate'=> date('Y-m-d H:i:s'),
				'description'=>'POS '.$sale_id,
				'type'=>'Sales',
				'employee_id' => $employee_id,	
				...
				'cost' => $coster);
			$this->db->insert('accounts',$sales_cash_data);
			
			if($customer_id != -1)
			{
				$current_balance = $this->Account->get_customer_balance($customer_id);
				$current_amount = bcadd($current_balance, $saled_amount);
				$c_balance = bcsub($current_amount, $payed);
				$sales_account_data = array(
					'thedate'=> date('Y-m-d H:i:s'),
					'description'=>'SINV - POS '.$sale_id,
					'employee_id' => $employee_id,	
					'account_id' => $customer_id,		
					'debit_amount'=> $saled_amount,
					'credit_amount' => $payed,
					'balance_amount'=> $c_balance);
				$this->db->insert('sales_accounts',$sales_account_data);
			}
	...
	public function delete($sale_id, $employee_id, $update_inventory = TRUE)
	{
		...
		$this->db->delete('accounts', array('description' => 'POS '.$sale_id));
		
		$this->db->delete('sales_accounts', array('description' => 'SINV - POS '.$sale_id));
		
		$this->db->delete('sales_payments', array('sale_id' => $sale_id));
		
		$this->db->delete('sales_items_taxes', array('sale_id' => $sale_id));
		
		$this->db->delete('total_tax', array('description' => 'POS '.$sale_id));
	...
	public function delete_vats($sale_id)
	{
		$success = FALSE;
		$this->db->trans_start();
		...
		$this->db->where_in('description', $sale_id);
		$success = $this->db->update('total_tax', array('deleted'=>1));
	 ...
	 */
	public function get_payment_options($giftcard = TRUE, $reward_points = FALSE, $enable_credit = FALSE)
	{
		...
		{
			$payments[$this->lang->line('sales_as_credit')] = $this->lang->line('sales_as_credit');
		}
	...
	public function create_temp_table(array $inputs)
	{
		...
		/**
	 * Retrieves all workorders that are in a suspended state
	 */
	public function get_all_workorders($customer_id = NULL, $search, $filters, $limit, $offset, $sort, $order)
	{
		if($customer_id == -1)
		{
			$query = $this->db->query("select sale_id, case when sale_type = '".SALE_TYPE_QUOTE."' then quote_number when sale_type = '".SALE_TYPE_WORK_ORDER."' then work_order_number else sale_id end as doc_id, sale_id as suspended_sale_id, sale_status, sale_time, dinner_table_id, customer_id, comment from "
				. $this->db->dbprefix('sales') . ' where sale_status = ' . SUSPENDED .' AND sale_type = ' . SALE_TYPE_WORK_ORDER);
		}
		else
		{
			$query = $this->db->query("select sale_id, case when sale_type = '".SALE_TYPE_QUOTE."' then quote_number when sale_type = '".SALE_TYPE_WORK_ORDER."' then work_order_number else sale_id end as doc_id, sale_status, sale_time, dinner_table_id, customer_id, comment from "
				. $this->db->dbprefix('sales') . ' where sale_status = '. SUSPENDED .' AND customer_id = ' . $customer_id.' AND sale_type = ' . SALE_TYPE_WORK_ORDER);
		}
	...
	public function get_last_sale_ids()
	{
		$this->db->from('sales');
    ...
    public function get_amount($sale_id)
	{
		$this->db->from('sales_payments');
		$this->db->where('sale_id', $sale_id);
	...
	public function get_sale_item_info($item_id, $customer_id)
	{
		$this->db->select('sales_items.item_unit_price AS item_unit_price');
		$this->db->select('sales_items.quantity_purchased AS quantity_purchased');
		
		$this->db->select('sales.sale_time AS sale_time');
		$this->db->from('sales_items AS sales_items');
        $this->db->join('sales AS sales', 'sales.sale_id = sales_items.sale_id', 'LEFT'); 
		$this->db->like('sales.customer_id', $customer_id);
		$this->db->like('sales_items.item_id', $item_id);
		$this->db->order_by(sale_time, "desc");
		$this->db->limit(10);
		return $this->db->get();
	}
	public function get_sale_all_item_info($item_id)
	{
		$this->db->select('sales_items.item_unit_price AS item_unit_price');
		$this->db->select('sales_items.quantity_purchased AS quantity_purchased');
	...
	public function getWorkOrderData(array $inputs)
	{
