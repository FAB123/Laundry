...
class Item extends CI_Model
{
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		...
		else
		{
			...
			$this->db->select('MAX(items.minimum_price) AS minimum_price');
			...
			$this->db->select('MAX(items.unit_type) AS unit_type');
			...
			{
				$this->db->select('MAX(item_damages.item_id) AS qty_item_id');
				$this->db->select('MAX(item_damages.location_id) AS location_id');
				$this->db->select('MAX(item_damages.quantity) AS quantity');
			}
			elseif($filters['stock_location_id'] > -1)
			{
		...
		if($filters['is_damaged'])
		{
			$this->db->join('item_damages AS item_damages', 'item_damages.item_id = items.item_id');
			
			if($filters['stock_location_id'] > -1)
			{
				$this->db->where('item_damages.location_id', $filters['stock_location_id']);
			}
		}
		elseif($filters['stock_location_id'] > -1)
		{
	...
	*/
	...
	public function check_low_sell_item_name($items_id)
	{
		$this->db->from('items');
		$this->db->where('low_sell_item_id', $items_id);
		$this->db->where('deleted', '0');

		return ($this->db->get()->num_rows() <= 1);
	}
	
	public function get_pack_ids($items_id, $rows = 0, $limit_from = 0)
	{
		$this->db->from('items AS items');
		$this->db->select('items.item_id AS item_id');
		$this->db->select('items.qty_per_pack AS qty_per_pack');
		$this->db->select('items.cost_price AS cost_price');
		
		$this->db->where('items.item_type', '4');
		$this->db->where('items.deleted', 0);
        $this->db->where('items.low_sell_item_id', $items_id);
	...
	public function check_pack_exist($items_id)
	{
		$this->db->from('items');
		$this->db->where('low_sell_item_id', $items_id);
	...
	public function change_pack_cost_price($item_id, $new_price)
	{
		$data = array('cost_price' => $new_price);
		return $this->save($data, $item_id);
	}
	
	public function insert_damage_goods($item_damages_data)
	{
		return $this->db->insert('item_damages', $item_damages_data);
	}
}
?>
