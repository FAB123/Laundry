...
class Customer extends Person
{
	...
	*/
	...
	public function get_all_by_location($location_id, $rows = 0, $limit_from = 0)
	{
		$this->db->from('customers');
		$this->db->join('people', 'customers.person_id = people.person_id');
		$this->db->where('deleted', 0);
		$this->db->where('customers.stock_location', $location_id);
		$this->db->order_by('last_name', 'asc');
	...
	*/
	public function get_search_suggestions($search, $stock_location = FALSE, $unique = TRUE, $limit = 25)
	{
		$suggestions = array();
	...
	*/
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'last_name', $order = 'asc', $stock_location, $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
