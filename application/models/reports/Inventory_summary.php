...
class Inventory_summary extends Report
{
	public function getDataColumns()
	{
		return array(array('item_name' => $this->lang->line('reports_item_name')),
					...
					array('wholesale_price' => $this->lang->line('reports_wholesale_price'), 'sorter' => 'number_sorter'),
					array('subtotal' => $this->lang->line('reports_sub_total_value'), 'sorter' => 'number_sorter'));
	}

	public function getData(array $inputs)
	{
		$this->db->select($this->Item->get_item_name('name') . ', items.item_number, item_quantities.quantity, item_quantities.quantity, items_taxes.percent, (item_quantities.quantity * items.qty_per_pack) as low_sell_quantity, items.reorder_level, stock_locations.location_name, items.cost_price, items.unit_price, items.wholesale_price, (items.cost_price * item_quantities.quantity) AS sub_total_value');
		$this->db->from('items AS items');
		$this->db->join('item_quantities AS item_quantities', 'items.item_id = item_quantities.item_id');
		$this->db->join('items_taxes AS items_taxes', 'items.item_id = items_taxes.item_id');
	...
	public function getSummaryData(array $inputs)
	{
		...
		foreach($inputs as $input)
		{
			...
			$tax_amount = ($input['sub_total_value']/100) * $input['percent'];
			$return['total_tax_percent'] += $tax_amount + $input['sub_total_value'];
