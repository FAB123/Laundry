...
$(document).ready(function()
{
    ...
    {
		swal({
			title: "<?php echo $this->lang->line("items_barcode_qty"); ?>",
			type: "input",
			showCancelButton: true,
			closeOnConfirm: false,
			inputPlaceholder: "<?php echo $this->lang->line("items_barcode_qty_place_holder"); ?>"
			}, function (inputCount) {
			    if (inputCount === false) return false;
				if (inputCount === "") {
				    swal.showInputError("<?php echo $this->lang->line("items_barcode_qty_valid_qty"); ?>");
					return false
				}
				
				if($.isNumeric(inputCount)) { 
				    var label_count = parseInt(inputCount);
					window.open(
						'index.php/items/generate_barcodes/'+label_count+'/'+table_support.selected_ids().join(':'),
						'_blank' // <- This is what makes it open in a new window.
					);
					swal.close()
				} else { 
					swal.showInputError("<?php echo $this->lang->line("items_barcode_qty_valid_number"); ?>");
					return false
				}		  
		});
    });
...
<div class="app-title">
      <div>
          <h1><i class="fa fa-tasks"></i> <?php echo $this->lang->line("module_" . $controller_name) ?></h1>
      </div>
