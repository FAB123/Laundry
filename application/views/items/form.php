...
<?php echo form_open('items/save/'.$item_info->item_id, array('id'=>'item_form', 'enctype'=>'multipart/form-data', 'class'=>'form-horizontal')); ?>
	<fieldset id="item_basic_info">
			...
			<div class="col-xs-8">
				...
				<label class="radio-inline">
					<?php echo form_radio(array(
							...
							'id'=>'item_type',
							'value'=>4,
							'checked'=>$item_info->item_type == ITEM_PACK)
					); ?> <?php echo $this->lang->line('items_pack'); ?>
				</label>
			</div>
		</div>
		<?php endif; ?>
		...
		<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('items_minimum_price'), 'minimum_price', array('class'=>' control-label col-xs-3')); ?>
			<div class="col-xs-4">
				<div class="input-group input-group-sm">
					<?php if (!currency_side()): ?>
						<span class="input-group-addon input-sm"><b><?php echo $this->config->item('currency_symbol'); ?></b></span>
					...
					<?php echo form_input(array(
							...
							'id'=>'minimum_price',
							'class'=>'form-control input-sm',
							'value'=>to_currency_no_money($item_info->minimum_price))
							);?>
					<?php if (currency_side()): ?>
						<span class="input-group-addon input-sm"><b><?php echo $this->config->item('currency_symbol'); ?></b></span>
					<?php endif; ?>
		...
		<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('items_wholesale_price'), 'wholesale_price', array('class'=>'required control-label col-xs-3')); ?>
			<div class='col-xs-4'>
				<div class="input-group input-group-sm">
					<?php if (!currency_side()): ?>
						<span class="input-group-addon input-sm"><b><?php echo $this->config->item('currency_symbol'); ?></b></span>
					...
					<?php echo form_input(array(
							...
							'id'=>'wholesale_price',
							'class'=>'form-control input-sm',
							'value'=>to_currency_no_money($item_info->wholesale_price))
							);?>
					<?php if (currency_side()): ?>
						<span class="input-group-addon input-sm"><b><?php echo $this->config->item('currency_symbol'); ?></b></span>
					<?php endif; ?>
		...
		<div class="form-group form-group-sm">
			...
			<div class='col-xs-1'>
				...
				<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('items_is_sp'), 'sp', array('class'=>'control-label col-xs-3')); ?>
			<div class='col-xs-1'>
				<?php echo form_checkbox(array(
						'name'=>'sp',
						'id'=>'sp',
						'value'=>1,
						'checked'=>($item_info->sp) ? 1 : 0)
						);?>
			</div>
		</div>
		
		<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('items_qty_type'), 'unit_type', array('class' => 'control-label col-xs-3')); ?>
			<div class='col-xs-4'>
				<?php echo form_dropdown('unit_type', array(
					'PCS' => $this->lang->line('items_qty_pcs'),
					'KG' => $this->lang->line('items_qty_kg'),
					'MTR' => $this->lang->line('items_qty_mtr'),
					'LTR' => $this->lang->line('items_qty_ltr')
				),
				$this->config->item('unit_type'), array('class' => 'form-control input-sm'));
				?>
			</div>
		</div>
		...
		<div class="form-group form-group-sm">
				<?php echo form_label($this->lang->line('items_custom_label'), 'custom_label', array('class'=>'control-label col-xs-3')); ?>
				<div class='col-xs-8'>
					<?php echo form_input(array(
							'name'=>'custom_label',
							'id'=>'custom_label',
							'class'=>'form-control input-sm',
							'value'=>$item_info->custom_label)
					);?>
				</div>
		</div>
	...
	$('#item_form').validate($.extend({
		...
		rules:
		{
			...
			low_sell_item_name:
			{
				remote:
				{
					url: "<?php echo site_url($controller_name . '/check_low_sell_item_name') ?>",
					type: 'POST',
					data: {
						'items_id' : function()
						{
							return $("input[name='low_sell_item_id']").val();
						},
					}
				}
			},
		...
		messages:
		{
			...
			low_sell_item_name: "<?php echo $this->lang->line('items_pack_duplicate'); ?>",
